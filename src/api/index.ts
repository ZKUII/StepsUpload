...
const MiApi = {
    getAccessCode: function (account: string, password: string, call: (response: any) => void) {
        let response;
        ...
        const options: AxiosRequestConfig = {
            method: 'POST',
            headers: { "User-Agent": "MiFit/4.6.0 (iPhone; iOS 14.0.1; Scale/2.00)", "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8" },
            params: params,
            url: url,
            maxRedirects: 0
        };
        axios(options).then(call).catch((res) => {
            call(res.response);
        });
        return response;
    },
    login: function (access: string, call: (response: any) => void): void {
        const url = "https://account.huami.com/v2/client/login"
        const params = new URLSearchParams();
        params.append('app_version', '4.6.0');
        params.append('code', access);
        params.append('country_code', 'CN');
        params.append('device_id', '2C8B4939-0CCD-4E94-8CBA-CB8EA6E613A1');
        params.append('device_model', 'phone');
        params.append('grant_type', 'access_token');
        params.append('third_name', 'huami_phone');
        params.append('app_name', 'com.xiaomi.hm.health');
        const options: AxiosRequestConfig = {
            method: 'POST',
            headers: { "User-Agent": "MiFit/4.6.0 (iPhone; iOS 14.0.1; Scale/2.00)", "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8" },
            params: params,
    ...
    updateStep: function (userId: string, appToken: string, step: number, call: (response: any) => void): void {
        ...
        axios.post(url, params, options).then(call).catch((res) => {
            console.log(res);
            call(res.response)
        });
    }
...
export default {
    updateStep: function (account: string, passowrd: string, step: number, call: (res: any) => void): void {
        MiApi.getAccessCode(account, passowrd, function (response) {
            let access = new URLSearchParams((response.headers.location as string)).get("access");
            MiApi.login(access!, function (response) {
                const data = response.data;
                const loginToken = data.token_info.login_token;
                const userId = data.token_info.user_id;
                MiApi.getApptoken(loginToken, function (response) {
                    const appToken = response.data.token_info.app_token;
                    MiApi.updateStep(userId, appToken, step, function (response) {
                        call(response);
                    })
                })
            });
